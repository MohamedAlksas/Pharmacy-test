// MaterialService now acts as a thin compatibility shim.
// All CRUD goes through ProductProvider; this class reads the same cached list
// so existing callers (AlertService, DashboardView, ReportsPage) continue to work.
...
class MaterialService {
  ...
  static List<MaterialModel> getAllMaterials() => List.unmodifiable(_cache);
  ...
  static List<MaterialModel> getExpiringSoonMaterials() =>
      _cache.where((m) {
        try {
          final diff =
              DateTime.parse(m.expiryDate).difference(DateTime.now()).inDays;
